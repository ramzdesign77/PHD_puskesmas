            ...
            <table class="data-table mx-auto w-full min-w-[1100px]">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="whitespace-nowrap px-6 py-5"><span class="badge {{ $user->is_active ? 'badge-resolved' : 'bg-gray-100 text-gray-500' }}"><i class="fas fa-circle text-[7px]"></i> {{ $user->is_active ? 'Aktif' : 'Nonaktif' }}</span></td>
                        <td class="px-6 py-5 text-right"><div class="flex justify-end gap-2"><a href="{{ route('users.show', $user) }}" class="btn-secondary btn-sm"><i class="fas fa-eye"></i></a><a href="{{ route('users.edit', $user) }}" class="btn-secondary btn-sm"><i class="fas fa-pen"></i></a><form action="{{ route('users.reset-password', $user) }}" method="POST" onsubmit="return confirm('Reset password menjadi password123?')">@csrf<button class="btn-secondary btn-sm" type="submit"><i class="fas fa-key"></i></button></form><form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('Nonaktifkan dan hapus petugas ini?')">@csrf @method('DELETE')<button class="btn-secondary btn-sm text-red-600" type="submit"><i class="fas fa-trash"></i></button></form></div></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
