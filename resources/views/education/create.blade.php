...
<div class="max-w-3xl mx-auto fade-in">
    ...
    <form method="POST" action="{{ route('education.store') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-6 md:p-8 space-y-6">
        @csrf
        <div><h1 class="text-xl font-bold text-gray-800">Informasi Artikel</h1><p class="text-sm text-gray-500 mt-1">Isi konten yang mudah dipahami warga.</p></div>
        ...
        <div><label for="title" class="block text-sm font-semibold text-gray-700 mb-2">Judul</label><input id="title" name="title" value="{{ old('title') }}" required maxlength="255" class="w-full rounded-xl border-gray-200 focus:border-medical-red focus:ring-medical-red" placeholder="Contoh: Menjaga Air Bersih di Rumah"></div>
        <div><label for="category" class="block text-sm font-semibold text-gray-700 mb-2">Kategori</label><select id="category" name="category" required class="w-full rounded-xl border-gray-200 focus:border-medical-red focus:ring-medical-red"><option value="">Pilih kategori</option>@foreach(['Air Bersih', 'Sanitasi', 'DBD & Nyamuk', 'Persampahan', 'Limbah', 'PHBS'] as $category)<option value="{{ $category }}" @selected(old('category') === $category)>{{ $category }}</option>@endforeach</select></div>
        <div><label for="image" class="block text-sm font-semibold text-gray-700 mb-2">Gambar <span class="font-normal text-gray-400">(opsional, JPG/PNG/WebP maks. 2 MB)</span></label><input id="image" type="file" name="image" accept="image/jpeg,image/png,image/webp" class="block w-full rounded-xl border border-gray-200 text-sm text-gray-500 file:mr-4 file:border-0 file:bg-red-50 file:px-4 file:py-3 file:font-semibold file:text-medical-red"></div>
        <div><label for="content" class="block text-sm font-semibold text-gray-700 mb-2">Konten</label><textarea id="content" name="content" rows="14" required class="w-full rounded-xl border-gray-200 focus:border-medical-red focus:ring-medical-red" placeholder="Tulis isi artikel di sini...">{{ old('content') }}</textarea></div>
        <div class="flex justify-end gap-3 border-t border-gray-100 pt-6"><a href="{{ route('education.manage') }}" class="rounded-xl px-4 py-3 text-sm font-semibold text-gray-600 hover:bg-gray-50">Batal</a><button type="submit" class="btn-medical rounded-xl px-5 py-3 text-sm font-semibold"><i class="fas fa-save mr-2"></i>Simpan Artikel</button></div>
    </form>
</div>
@endsection
